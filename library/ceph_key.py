...
def run_module():
    ...
    if state == "present":
        ...
        entities = lookup_ceph_initial_entities(out, module)

        output_format = "plain"
        for entity in entities:
            key_path = build_key_path(entity)
            if key_path is None:
                fatal("Failed to build key path, no entity yet?", module)
            elif os.stat(key_path):
            ...
            rc, cmd, out, err = exec_commands(
                module, info_cmd.extend(extra_args))  # noqa E501
